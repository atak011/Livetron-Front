...
import { getEventPasts } from "../../LiveCommerce/Services/LiveCommerceService";
...
export default function EventPast() {
  ...
  const [eventPasts, setEventPasts] = useState<any[]>([]);

  const rows: any[] = [];
  React.useEffect(() => {
    const getEventRefLinkList = async () => {
      const eventPastsFromServer = await getEventPasts();
      console.log("eventlist from server", eventPastsFromServer);
      setEventPasts(eventPastsFromServer.data);
    };
