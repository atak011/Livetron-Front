...
import { getEventUpcomings } from "../../LiveCommerce/Services/LiveCommerceService";
...
export default function EventUpcoming() {
  ...
  const [eventUpcomings, setEventUpcomings] = useState<any[]>([]);

  const rows: any[] = [];
  React.useEffect(() => {
    const getEventRefLinkList = async () => {
      const eventUpcomingsFromServer = await getEventUpcomings();
      console.log("eventlist from server", eventUpcomingsFromServer);
      setEventUpcomings(eventUpcomingsFromServer.data);
    };

    getEventRefLinkList();
  }, []);

  eventUpcomings.map(item => {
    rows.push(
      createData(
        item.id,
        item.name,
